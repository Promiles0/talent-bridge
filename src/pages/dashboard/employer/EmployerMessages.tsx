...
export default function EmployerMessages() {
  ...
  const { data: profiles } = useQuery({
    queryKey: ["msg-profiles-emp", contactIds],
    queryFn: async () => {
  ...
  const getName = (id: string) => profiles?.find((p: any) => p.id === id)?.full_name || "User";
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center justify-between">
                      ...
                      {conv.unreadCount > 0 && (
                        <span className="h-5 min-w-5 px-1.5 text-[10px] rounded-full bg-primary text-primary-foreground flex items-center justify-center">
                          {conv.unreadCount}
                        </span>
                      )}
                    </div>
